...
lives-ok {
    my $v4 = WebService::AWS::V4.new(method => $get, body => '', uri => $uri_str, headers => @headers, region => $region, service => $service);
}, 'correctly initialize well-formed obj';

dies-ok {
    my $v4 = WebService::AWS::V4.new(method => '', body => '', uri => $uri_str, headers => @headers, region => $region, service => $service);
}, 'caught exception when trying to initialize with missing method';

dies-ok {
    my $v4 = WebService::AWS::V4.new(method => 'PUT', body => '', uri => $uri_str, headers => @headers, region => $region, service => $service);
}, 'caught exception when trying to initialize with bad method';

dies-ok {
    my $v4 = WebService::AWS::V4.new(method => $get, body => '', uri => '', headers => @headers, region => $region, service => $service);
}, 'caught exception when trying to initialize with missing uri';

dies-ok {
    my $v4 = WebService::AWS::V4.new(method => $get, body => '', uri => 'htt', headers => @headers, region => $region, service => $service);
}, 'caught exception when trying to initialize with malformed uri';

dies-ok {
    my $v4 = WebService::AWS::V4.new(method => $get, body => '', uri => 'https://iam.amazonaws.com/home/documents+and+settings?a/z=b&C=d', headers => @missing_host_header, region => $region, service => $service);
}, 'caught exception on missing host header';

dies-ok {
    my $v4 = WebService::AWS::V4.new(method => $get, body => '', uri => 'https://iam.amazonaws.com/home/documents+and+settings?a/z=b&C=d', headers => @malformed_headers, region => $region, service => $service);
...
lives-ok {
    my $v4 = WebService::AWS::V4.new(method => $get, body => '', uri => 'https://iam.amazonaws.com/home/documents+and+settings?a/z=b&C=d', headers => @headers, region => $region, service => $service);
...
dies-ok {
    my $v4 = WebService::AWS::V4.new(method => $get, body => '', uri => 'https://iam.amazonaws.com/home/documents+and+settings?ab&C=d', headers => @headers, region => $region, service => $service);
...
lives-ok {
    my $v4 = WebService::AWS::V4.new(method => $get, body => '', uri => $aws_sample_uri, headers => @aws_sample_headers, region => $region, service => $service);
